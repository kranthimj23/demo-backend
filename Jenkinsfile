...
pipeline {
    ...
    environment {
        ...
        ZONE = 'us-central1'
        GCP_KEY = '/var/lib/jenkins/keys/mobile-app-1-482109.json'
        PYTHON_EXEC = 'python3.11'
        GIT_CREDENTIALS_ID = credentials('jenkins-token')
    }

    stages {
        stage('Checkout demo-backend repo') {
            steps {
                deleteDir()
                script {
                    withCredentials([string(credentialsId: 'jenkins-token', variable: 'GIT_TOKEN')]) {
                        checkout([
                            ...
                            branches: [[name: "*/dev"]],
                            userRemoteConfigs: [[
                                url: "https://${GIT_TOKEN}@github.com/kranthimj23/demo-backend.git"
                            ]]
                        ])
                    }
                }
        ...
        stage('Build Docker Image') {
            steps {
                script {
                    image_repo = "us-central1-docker.pkg.dev/${env.PROJECT_ID}/demo-backend/backend"
